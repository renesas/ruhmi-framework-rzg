...
/*****************************************
* Function Name : draw_rgb_bbox
* Description   : Draw bounding box in RGB
* Arguments     : x_min = top left coordinate X of box to be drawn
...
* Return Value  : -
******************************************/
void Image::draw_rgb_bbox(uint32_t x_min, uint32_t y_min, uint32_t x_max, uint32_t y_max, uint32_t color)
{
...
void Image::draw_rect(int32_t x, int32_t y, int32_t w, int32_t h, uint32_t color)
{
    ...
    draw_rgb_bbox(x_min, y_min, x_max, y_max, color);
}
